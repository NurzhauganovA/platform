...
#
# Спрашивает подтверждение словом. Вместе с базой уходит журнал действий — то,
# чем отвечают на вопрос «кто это сделал», — и набирают эту команду обычно в
# спешке, когда всё уже встало. Суточные выгрузки журнала при этом остаются:
# они лежат каталогом на машине, а не томом.
clean:
	@echo "Это сотрёт базу платформы вместе с журналом действий."
	@echo "Выгрузки журнала в $${AUDIT_ARCHIVE:-./backups/audit} останутся."
	@printf "Наберите YES, чтобы продолжить: " && read answer && [ "$$answer" = "YES" ] || { echo "Отменено."; exit 1; }
	@$(COMPOSE) down -v

# Выгружает журнал действий за всё время одним файлом. Нужен перед переездом и
# перед `make clean`: суточные выгрузки делает прогон, а этой командой берут
# всё разом.
audit-dump:
	@mkdir -p $${AUDIT_ARCHIVE:-./backups/audit}
	@$(COMPOSE) exec -T postgres pg_dump -U fintend -d fintend -t audit_log --data-only \
		| gzip > $${AUDIT_ARCHIVE:-./backups/audit}/audit-full-$$(date +%Y%m%d-%H%M).sql.gz
	@ls -lh $${AUDIT_ARCHIVE:-./backups/audit} | tail -3
