...
sudo ./infra/fix-names.sh -y     # если проверка нашла расходящиеся имена
```
...
`fix-names.sh` разбирается с одним из её вердиктов — «есть на диске, но в
другой форме записи». Одну и ту же русскую букву Unicode позволяет записать
двумя способами, macOS ищет файл нечувствительно к этому, ext4 сравнивает
байты. Скрипт переименовывает файлы на диске под то, что записано в базе, а не
наоборот: тем же именем в базе связаны решения по закупкам и находки
поставщиков, и правка одного столбца дала бы базу, где документ открывается, а
себестоимости по нему нет.
