...
exclude=(
  ...
  # Второй перечень — коды с казахстанским производителем из книги отдела.
  # Здесь же и по той же причине: `.json` сейчас ничем не отсекается, но
  # запрет на него однажды добавят ради чужих `package.json`, и перечень
  # исчезнет из образа молча — платформа просто перестанет отмечать коды.
  --include "Перечень_КТП.json"
  --exclude ".env"
  --exclude ".env.local"
