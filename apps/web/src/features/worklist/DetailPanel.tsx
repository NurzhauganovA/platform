...
/**
 * Разделы, которые в режиме сведений остаются и после отсечения.
 *
 * По ключу, а не по заголовку: заголовок правят, ключ объявляет сервер. Лоты
 * объявления нужны и здесь — в закупке их бывает четыре, торги идут по
 * объявлению целиком, и знать об этом надо до того, как считать себестоимость
 * одного. Спецификация нужна затем же, зачем и всегда: по ней меряют
 * соответствие заявки, а лежит она в нашей базе и на скорость не влияет.
 */
const FACTS_KEEP = new Set(["announce_lots", "tech_spec"]);

/**
 * Какие разделы показать.
 *
 * В режиме сведений — по «Что покупают» включительно, плюс те, что оставлены
 * явно. Резать по названию, а не по числу разделов: у площадок их разное
 * количество, а «Что покупают» есть у всех и всегда стоит вторым. Названия
 * такого нет — показываем всё: пустая панель хуже лишнего раздела.
 *
 * Отсечение здесь — страховка, а не правило. Что показывать, решает сервер: у
 * госзакупок режим сведений отдаёт ровно нужные разделы и не ходит за ними на
...
function shown(sections: DetailSection[], facts: boolean): DetailSection[] {
  ...
  return sections.filter(
    (section, place) => place <= at || FACTS_KEEP.has(section.key),
  );
}
