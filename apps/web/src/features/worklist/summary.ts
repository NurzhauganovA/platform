...
/** «3 часа», «24 часа», «5 часов» — порог приходит числом, а склонение своё. */
function hours(count: number): string {
  const tail =
    count % 100 > 4 && count % 100 < 21
      ? 2
      : [2, 0, 1, 1, 1, 2][Math.min(count % 10, 5)];
  return `${count} ${["час", "часа", "часов"][tail]}`;
}
...
export function summarise({
  ...
  goodLabel: string;
  /** За сколько часов до конца приёма закуп считается горящим. Приходит с
   *  сервера: у площадок трое суток «горят» только на словах — за это время
   *  успевают и посчитать, и подать, — а на портале госзакупок тем же словом
   *  покрашена строка, и порог там свой. Зашитое здесь число расходилось с
   *  легендой цвета, и на экране стояли «Горит 2» и «Горит 6» рядом. */
  urgentHours?: number;
}): Tile[] {
  const tiles: Tile[] = [];
  ...
  if (rows.some((row) => row.deadline)) {
    ...
    tiles.push({
      ...
      hint: `приём закрывается в ближайшие ${hours(urgentHours)}`,
    });
  }
