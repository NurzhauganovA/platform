...
export function Passed({
  ...
  return (
    ...
    >
      ...
      00:00
      {/* Диктору — та же фраза, что в подсказке. Зашитый текст про приём
          заявок читался и на сроках обсуждения: незрячему сотруднику
          сообщали, что провалена подача, хотя приём ещё идёт. Видимого текста
          тут, кроме нулей, нет, а `title` диктор не читает — ошибку получали
          ровно те, для кого эта строка и написана. */}
      <span className="sr-only"> — {submitted ? done : missed}</span>
    </span>
  );
}
