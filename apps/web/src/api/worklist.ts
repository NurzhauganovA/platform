...
/**
 * Документ с требованиями заказчика к этой строке.
 *
 * Одним полем на три места: вкладка разбора собирает из него таблицу, вкладка
 * файлов даёт скачать, панель сведений показывает текст. Своя догадка в
 * каждом из них однажды дала бы три разных ответа на «а есть ли вообще
 * спецификация».
 */
export interface SpecFile {
  name: string;
  /** Откуда скачать — через нас. Пусто: файл записан, но ссылки на него нет. */
  url: string;
  /** Сколько знаков текста прочитано. Ноль — разбор моделью не соберётся. */
  chars: number;
}
...
export interface Detail {
  ...
  /** Техническая спецификация строки. Пусто — площадка её не забрала. */
  spec: SpecFile | null;
...
export const worklists = {
  ...
  /**
   * Чужие лоты объявления — те, что не подошли под наши коды ЕНС ТРУ.
   *
   * Отдельным запросом, потому что за ними идут на портал, а он отвечает
   * секундами: замер до этой правки дал 49 секунд на открытие панели против
   * 0,1 у того же разбора без портала. Панель показывает своё сразу, а этот
   * раздел встаёт на место, когда придёт.
   */
  neighbours: (slug: WorklistSlug, id: string) =>
    api.get<DetailSection>(
      `/api/${slug}/item/${encodeURIComponent(id)}/neighbours`,
    ),
