...
export interface Me {
   ...
   * Что человеку можно.
   *
   * По ним прячутся кнопки и данные: нажатая кнопка с отказом читается как
   * поломка, а человек к тому моменту уже уверен, что сделал дело.
   *
   * Защитой это не является. Права проверяются на эндпоинтах, и спрятать
   * кнопку, оставив эндпоинт открытым, — самый распространённый способ отдать
   * себестоимость наружу.
   */
  permissions: string[];
