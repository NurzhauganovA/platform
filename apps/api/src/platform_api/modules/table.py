...
class RowOut:
    ...
    code: str = ""
    """Постоянный код строки: «TN-00042».

    Им её и называют. Выдаётся один раз и остаётся при позиции: список
    пересобирается при каждом разборе, и место в нём сдвигается от каждой
    новой закупки, а код — нет.
    """
