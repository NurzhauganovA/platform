...
def sync_sources(
    ...
    if not has_session():
        return {
            "records": 0,
            "reason": (
                "Платформа не вошла в кабинет OMarket. Вход подтверждается"
                " ЭЦП в окне браузера, поэтому его делают на своей машине, а не"
                " на сервере. Обратитесь к тому, кто настраивал раздел."
            ),
            "cost_usd": 0.0,
        }
