    ...
    return Section(
        ...
        note=(
            ...
            if len(reachable) == len(files)
                ...
                "Часть файлов платформе не видна: архив закупок лежит на машине"
                " тендерщика и подключается к платформе томом. Названия видны и"
                " без него — по ним понятно, что в папке есть."
            )
        ),
    )
