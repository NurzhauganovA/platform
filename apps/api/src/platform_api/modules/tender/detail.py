...
def documents_of(folder_path: str, title: str) -> tuple[Field, ...]:
    """Документы позиции — по папке и названию, а не по строке отбора.

    Работа над лотом живёт своей жизнью: отбор пересобирается, строки меняют
    имена, а работа остаётся той же. Найти её документы можно только по тому,
    чем позиция и записана.

    Пусто, если закупка ушла из отбора — например, папку разобрали заново и
    позиция получила другое название. Это не ошибка: снабжение работает по
    тому, что записано в самой работе.
    """
    ...
    найдена = next(
        (
            item
            for item in ranked()
            if (item.row.folder_path or "") == folder_path and item.row.title == title
        ),
        None,
    )
    if найдена is None:
    ...
    return tuple(
        ...
        for item in case_files(item_id)
    )
...
def _missing_note(files: Any, reachable: Any) -> str:
    ...
    if _forbidden(where):
        return (
            f"Документы на месте, но платформе к ним закрыт доступ:"
            f" «{where}». Это права на папку — настройка сервера, покажите это"
            " сообщение администратору."
        )
    if reachable:
        return (
            f"Часть документов платформе не видна. Искали в «{where}» —"
            " похоже, при переносе на сервер доехали не все файлы. Это"
            " настройка сервера: покажите это сообщение администратору."
        )
    return (
        f"Ни одного файла не видно. Искали в «{where}» — похоже, папки закупок"
        " платформе не отданы или лежат в другом месте. Это настройка сервера:"
        " покажите это сообщение администратору. Названия документов видны и"
        " так — по ним понятно, что в папке есть."
    )
