...
def withdrawn_codes() -> frozenset[str]:
    """Коды ЕНС ТРУ, изъятые из национального режима — приказом Минпрома.

    Отвечает на портальный вопрос «Изъятие из национального режима: да или
    нет», и потому это не тот набор, которым красится код в тендерном отборе:
    там объединение с реестром Электронного магазина и вопрос другой — «есть ли
    казахстанский производитель». Код из реестра, в приказ не попавший, на
    портале значится «Нет», и участвовать в такой закупке можно.

    Живёт здесь, хотя нужен разделу госзакупок: перечень читает тендерное ядро,
    а обращение к чужому пакету идёт через один файл модуля. Второй читатель
    того же файла Word разошёлся бы с первым в день, когда пришлют обновление.
    """
    return frozenset(getattr(core_settings(), "withdrawn", frozenset()))
