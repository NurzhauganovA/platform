...
def ranked() -> list[Any]:
    """Собранный отбор. То же, что видит список, — из того же кэша.

    Нужно наружу: работа над лотом берёт из отбора позиции и находки, и
    второй сборкой это обошлось бы в девять секунд на каждое нажатие.
    """
    return list(_ranked())
...
def mark_cell(title: str, item: RankedRow) -> str:
    ...
    Пока это одна ячейка — код ЕНС, по которому есть казахстанский
    производитель. Красится он сам, а не строка целиком: заливка строки занята
    вердиктом, и два смысла в один цвет не вложить. Зато красный код замечают —
    он стоит первой колонкой, и его видно, не читая остального.

    Чем именно он красен, написано в разборе строки (`detail._ens`): цвет сам
    по себе ничего не объясняет, а легенда над таблицей — про вердикт.
    """
    return "critical" if title == "ЕНС ТРУ" and is_domestic(item.row) else ""


def is_domestic(row: Any) -> bool:
    """Есть ли по коду ЕНС казахстанский производитель.

    Перечень читает ядро, и их там два: приказ Минпрома файлом Word и перечень
    Электронного магазина, собранный из цветной книги отдела
    (`tender-analyze ktp`). Совпадают они меньше чем наполовину, а вопрос
    задают один, поэтому ядро отдаёт объединение.

    Разбирать перечни здесь значило бы завести второй список, который
    разойдётся с первым в день, когда пришлют обновление. Хуже того, тем же
    множеством ядро красит код в выгруженной книге: сотрудник сверяет экран с
    книгой, и расхождение в один код — это полчаса выяснений, кто из двух прав.
