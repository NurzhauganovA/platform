...
ESSENTIAL: tuple[str, ...] = (
    # Код ЕНС — не справочная мелочь: по нему закупка сверяется с перечнями
    # казахстанских производителей, и красная ячейка объясняется именно им.
    # Цвет без видимой причины заставляет открывать разбор ради одного поля —
    # поэтому там же, в разборе, рядом с кодом стоит оговорка, что он значит.
    "ЕНС ТРУ",
    "название закупки",
    "название заказчика",
    "категория",
