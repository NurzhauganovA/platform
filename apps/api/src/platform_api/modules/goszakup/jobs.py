...
def fetch_lot(ctx: JobContext, *, number: str = "", **_: Any) -> dict[str, Any]:
    ...
    if not outcome.found:
        ...
        #
        # Но «объявление есть, лотов портал не публикует» — это третий ответ, и
        # он важнее обоих: искать опечатку там незачем, закупка открыта у
        # человека в соседней вкладке.
        return {
            "number": искомое,
            "by": outcome.by,
            "found": 0,
