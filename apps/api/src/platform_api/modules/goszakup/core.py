...
def is_ktp(row: Any) -> bool:
    """Изъят ли товар лота из национального режима — то, что портал зовёт КТП.

    Считается по коду ЕНС ТРУ, а не читается с портала. На портале признак
    есть, но только в кабинете за ЭЦП: открытое API его не отдаёт ни у лота, ни
    у объявления — проверено перебором всех полей. Зато сам перечень портал
    публикует файлом, и это ровно тот файл, который читает тендерное ядро:
    совпадает побайтово, вместе с приказом, которым утверждён.

    Отвечает приказ Минпрома, а не объединение перечней: реестр Электронного
    магазина — про наличие производителя, и код из него портал изъятым не
    считает.
    """
    code = (getattr(row, "enstru_code", "") or "").strip()
    if not code:
        return False
    from platform_api.modules.tender.core import withdrawn_codes

    return code in withdrawn_codes()


def mark_cell(title: str, row: Any) -> str:
    """Отмечает ячейку кода ЕНС ТРУ, когда товар изъят из национального режима.

    Красится ячейка, а не строка: заливка строки занята сроком приёма, и два
    смысла в один цвет не вложить. Что значит красный код, написано в разборе
    строки — цвет сам по себе ничего не объясняет.
    """
    return "critical" if title == "Код ЕНС ТРУ" and is_ktp(row) else ""


def note_cell(title: str, row: Any) -> str:
    """Слово при отметке — «КТП».

    Одного цвета мало: при дальтонизме закрашенная ячейка неотличима от
    обычной, а в тендерном отборе за тем же красным кодом ходили спрашивать и
    люди с обычным зрением — цвет в списке не объясняет ничего. Слово ровно то,
    которым это названо на портале и в отделе.
    """
    return "КТП" if title == "Код ЕНС ТРУ" and is_ktp(row) else ""
