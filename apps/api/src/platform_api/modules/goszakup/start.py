...
CODE_WIDTH = 1
CODE_SEPARATOR = ""
"""Код строки портала: «GZ2», «GZ10», «GZ1000».

Без ведущих нулей. «GZ000002» человек читает по слогам и пересчитывает нули,
чтобы не спутать с «GZ000020», — а код называют вслух на планёрке и пишут в
переписке. Нули стояли ради того, чтобы коды выравнивались столбиком и
сортировались как текст: выравнивание держит моноширинный шрифт в колонке, а
по коду ничего не сортируется — списки идут по сроку.

Ширина здесь единица, потому что формат собирается одним и тем же кодом на все
разделы, и «ноль знаков» он не понимает; на номере это не сказывается —
`f"{2:01d}"` даёт «2», а `f"{1000:01d}"` — «1000».

Тендерная приставка при этом осталась пятизначной: «TN-00042» выдана давно, ею
закупки называют вслух и в переписке, и менять её форму значило бы разослать
всем новые имена для того же самого."""
...
def ensure(
    ...
    *,
    ...
    Срок берётся портальный, когда он есть, и считается от публикации, когда
    его нет: два рабочих дня, как их даёт закон. Портальный бывает не у всякой
    закупки — у конкурсов он заполнен, у запроса ценовых предложений пуст, — и
    приходит только в карточке объявления, а не в строке поиска. Раньше вместо
    него подставлялось окончание приёма заявок, и «осталось» показывало неделю
...
def discussion_deadline(lot: Any) -> datetime | None:
    ...
    Портальный срок сильнее расчётного: свой расчёт — замена, а не правило.
    Долго он и был единственным, потому что поля приходили пустыми: разбор не
    понимал их вида. В строке поиска лотов портал печатает время строкой с
    зоной, а в карточке объявления те же поля — числом секунд с начала эпохи,
    и `discussion_end_date` молча становился ничем. Все 529 конкурсов лежали в
    базе без срока обсуждения при заполненном поле на портале.

    Заполнен он не везде: у запроса ценовых предложений обсуждения нет, и там
    расчёт остаётся единственным ответом.
