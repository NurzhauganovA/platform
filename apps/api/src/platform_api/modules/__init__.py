...
class NavItem:
    ...
    roles: tuple[str, ...] = ()
    """Кому пункт виден — по встроенным ролям. Оставлено для модулей, которые
    ещё не перешли на права; новые пункты объявляют `permission`."""

    permission: str = ""
    """Право, без которого пункт не показывается.

    Правом, а не ролью: роли заводит администратор, и список имён новую роль
    не знает — пункт пропадал бы у неё, хотя доступ ей выдали. Одно право на
    страницу: «Лоты портала» и «Обход портала» лежат в одном модуле, но первое
    — ежедневная работа, второе — настройка.

    Права всё равно проверяются на эндпоинтах: скрытый пункт меню — это
...
class ModuleSpec:
    ...
    on_take: Callable[..., tuple[uuid.UUID, ...]] | None = None
    """Что запустить, когда лот этой площадки взяли в работу.

    Объявляет модуль, а не оболочка: «взять в работу» у площадок значит разное.
    У госзакупок за нажатием стоят два платных прогона — написать замечание и
    разобрать спецификацию, — а у тендерного отбора закупка приходит папкой по
    почте, и обсуждать её не с кем.

    Возвращает задачи, которые надо отдать очереди. Не ставит их сам: до
    `commit` исполнитель их не найдёт, а фиксирует транзакцию обработчик.

    Получает сессию, карточку, человека, настройки и Redis — всё именованными
    параметрами, чтобы добавление ещё одного не переписывало все площадки.
    """
