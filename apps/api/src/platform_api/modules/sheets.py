...
WIDTH = {SUBJECT: 100, DEMAND: 340, BRIEF: 170, PRICE: 110, OURS: 220, OUR_SPEC: 300}
...
def hand_columns() -> tuple[Column, ...]:
    ...
    Цена, наш товар и наша техническая спецификация — то, ради чего разбор и
    затевается: строка требования нужна, чтобы напротив неё встало наше
    предложение. Столбцы человеческие, поэтому пересборка их не трогает, а
    убрать их с экрана по-прежнему можно крестиком.

    «Наш товар» — марка и модель: что именно предлагаем. «Наш ТС» — чем это
    отвечает требованию заказчика: характеристики нашего предложения, которые
    потом сверяют со спецификацией строка за строкой. Два столбца, а не один,
    потому что вопросы разные: первый спрашивает «что», второй — «подходит ли».

    Себестоимости здесь больше нет. Она стояла рядом с ценой ради заработка из
    разницы, но заполнять её оказалось нечем: цены поставщиков платформа по
    госзакупкам не знает, а снабжение работает в своей копии таблицы — и
    столбец стоял пустым во всех разборах.
    """
    return (
        Column(key=PRICE, title="Цена", width=WIDTH[PRICE]),
        Column(key=OURS, title="Наш товар", width=WIDTH[OURS]),
        Column(key=OUR_SPEC, title="Наш ТС", width=WIDTH[OUR_SPEC]),
    )
...
def save(
    ...
    *,
    ...
    # Разметка ячеек вычищается здесь, а не только в браузере: в базу пишет
    # запрос, а не страница, и вставленный `<script>` исполнился бы с нашего
    # адреса по нажатию коллеги — то есть с его сессией, ценами и маржой.
    sheet.rows = [
        {"key": row.key, "cells": {key: markup.tidy(value) for key, value in row.cells.items()}}
        for row in rows
    ]
    db.flush()
    return _out(sheet)
