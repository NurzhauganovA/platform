...
class CustomRole(Base, UUIDPrimaryKey, Timestamps):
    """Роль, заведённая администратором в платформе.

    Встроенных десять, и они описывают отделы так, как они сложились у нас.
    Компания молодая: появляется «снабженец без цен», «стажёр тендерного
    отдела», «бухгалтер, которому нужны только итоги». Прибивать каждую такую
    роль в код значит выкладывать платформу ради одного человека.

    Права хранятся списком, а сам список прав определён кодом
    (`auth/permissions.py`). Так нельзя выдать право, которого никто не
    проверяет: набор закрыт, и роль может только выбирать из него.

    Встроенные роли здесь не лежат. Их права заданы кодом и не правятся: это
    ответ на вопрос «что сломается, если поменять», который иначе пришлось бы
    выяснять на работающей платформе. Нужна своя — заводится рядом.
    """

    __tablename__ = "custom_roles"
    __table_args__ = (UniqueConstraint("organization_id", "key", name="custom_role_key"),)
    ...
    """Ключ: латиницей, без пробелов. Попадает в журнал действий и в ответы
    API, а кириллица в них превращается в проценты с цифрами."""
...
class Membership(Base, UUIDPrimaryKey, Timestamps):
    ...
    """Встроенная роль. При своей роли здесь стоит «Наблюдатель».

    Наблюдатель, а не пусто: колонка была и остаётся обязательной, а
    самостоятельная роль своих прав в неё не уложит — их набор произвольный.
    Наблюдатель выбран как самое безопасное значение: если какое-то место ещё
    смотрит на встроенную роль, а не на права, человек получит меньше
    доступа, а не больше."""

    custom_role_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("custom_roles.id", ondelete="SET NULL"), nullable=True, index=True
    )
    """Своя роль, если человеку выдали её. Права тогда берутся из неё.

    `SET NULL` намеренно: удалённая роль не уносит человека из платформы — он
    остаётся с правами наблюдателя, и администратор видит это на экране людей."""

    custom_role: Mapped[CustomRole | None] = relationship(lazy="selectin")
...
class DiscussionMessage(Base, UUIDPrimaryKey, Timestamps):
    ...
    """Кого позвали этой репликой: идентификаторы сотрудников и `all` — всех.

    Списком в самой реплике, а не разбором текста при показе. Имя человека в
    тексте — это буквы: сотрудник меняет фамилию, однофамильцы приходят вдвоём,
    и «кому ушло уведомление» перестаёт быть ответимым вопросом ровно тогда,
    когда его задают, — после того как позвали не того.

    `all` хранится словом, а не развёрнутым списком людей: позвали всех, а не
    тех тринадцать, кто работал в тот вторник."""
...
class LotStatus(StrEnum):
    ...
    Шесть состояний, а не четырнадцать. Прежний список описывал путь по
    отделам: «Обсуждение», «На разборе», «Готов к участию», «Договор»,
    «Исполнение», «Ожидаем оплату». На планёрке по нему отвечали не на тот
    вопрос: он говорил, чья сейчас очередь, а спрашивают — где закупка. Чья
    очередь, видно по задачам и по четырём точкам отделов в строке, и держать
    то же самое ещё и статусом значило переводить лот пять раз за день руками.

    Работа отделов идёт внутри «В работе». Обсуждение, разбор, юрист, технолог
    — всё это один статус: они работают параллельно и в любом порядке, а
    статус, который движется по ним по очереди, врёт ровно в тот момент, когда
    юрист пишет замечание, а снабжение уже ищет товар.

    Итог закупки — не статус. «Выиграли» и «Проиграли» стояли в этом же списке,
    и лот, который мы завершили не участвуя, попадал в «Не участвуем» — то есть
    в состояние, из которого он уже не двигался, хотя работа над ним просто
    кончилась. Теперь итог отдельной величиной (`LotOutcome`): завершённый лот
    бывает выигранным, проигранным и никаким.

    Порядок объявления — порядок жизни. По нему строится полоса хода в карточке
    и сортировка списка.
    """
    ...
    APPROVAL = "approval"
    """На согласовании: пять отделов подтверждают, что берёмся."""

    SUBMISSION = "submission"
    """Подача. Подписи собраны, заявку готовят и подают через портал."""

    WAITING = "waiting"
    """Подали, ждём протокол итогов."""

    DONE = "done"
    """Завершённый. Чем кончилось — в `outcome`."""


class LotOutcome(StrEnum):
    """Чем кончилась закупка по протоколу итогов.

    Отдельно от статуса, потому что это разные вопросы. Статус отвечает «где
    лот», итог — «что вышло»; пока они лежали в одном списке, завершённый и
...
class LotCard(Base, UUIDPrimaryKey, Timestamps):
    ...
    outcome: Mapped[LotOutcome] = mapped_column(
        ...
        server_default=LotOutcome.NONE.name,
        index=True,
    )
    """Чем кончилась закупка по протоколу итогов.

    Отдельно от статуса: завершённый лот бывает выигранным, проигранным и
    никаким. Пока итог стоял статусом, «Проиграли» и «Завершён» были
    взаимоисключающими, хотя проигранный лот завершён ровно так же."""
    ...
    bid_amount: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
    """За сколько подали заявку. Пусто — заявку ещё не подавали.

    Наша цена участия, а не цена закупки (`amount`) и не цена победителя
    (`won_amount`). Три разные суммы, и путать их дорого: по первой считают
    маржу, вторая объявлена заказчиком, третья говорит, насколько мы
    промахнулись.

    Вводится тендерщиком при подаче и дальше не меняется сама. По ней лот и
    считается поданным: отметка времени рядом (`submitted_at`) отвечает
    «когда», а эта — «за сколько», и без второй первая бесполезна: через месяц
    спрашивают именно сумму.
    """
    ...
    status_at: Mapped[datetime | None] = mapped_column(nullable=True, index=True)
    """Когда статус менялся последний раз.

    По нему идёт сортировка списков: сверху то, что двигалось только что. До
    сих пор список сортировался по сроку приёма заявок, и лот, который вчера
    перевели на согласование, лежал в середине между теми, к которым никто не
    прикасался, — а на планёрке спрашивают «что нового», и ответом было
    листание.

    Отдельным полем, а не выводом из ленты событий: лента лежит другой
    таблицей, и сортировка списка по ней означала бы соединение на каждое
    открытие страницы ради одного столбца."""
...
class Approval(Base, UUIDPrimaryKey, Timestamps):
    ...
    by_name: Mapped[str] = mapped_column(String(255), default="")
    """Кто подписал — именем, а не только ссылкой.

    Вопрос «кто это одобрил» возникает ровно тогда, когда закупка вышла в
    убыток, и это может быть через полгода после того, как человек уволился и
    его запись удалили. Ссылка к тому времени обнулена, а подпись должна
    остаться подписью."""
...
class LotEvent(Base, UUIDPrimaryKey, Timestamps):
    ...
    """Имя на момент действия.

    Хранится копией рядом с ссылкой на человека. Ссылка при удалении сотрудника
    обнуляется (`SET NULL`) — иначе удаление унесло бы саму запись, — и без этой
    копии лента превращалась бы в список действий без авторов: «перевёл в
    „Подача“» без имени неотличимо от прогона по расписанию. А спрашивают по
    ленте ровно одно: кто это сделал.

    Имя на момент действия, а не сегодняшнее: человек меняет фамилию, и запись
    должна остаться той, какой её прочитали тогда."""
