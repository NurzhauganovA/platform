...
class WriterSettings(BaseModel):
    ...
    """Потолок ответа для замечания. Замечание — страница текста, и больше ему
    не нужно."""

    sheet_tokens: int = Field(default=32_000, ge=4_096, le=64_000)
    """Потолок ответа для разбора спецификации.

    Свой и вчетверо больше: разбор переносит требования заказчика дословно,
    поэтому ответ растёт вместе с исходником. На сервере из четырнадцати
    страниц ответ упирался в восемь тысяч токенов и обрывался посреди таблицы —
    а выглядело это как «модель ответила не таблицей».

    Потолок, а не запрос: сколько просить, считает `sheeting.budget` по длине
    самой спецификации, и на короткой закупке лишнего не берётся."""
